// frontend/web/src/modules/admin/pages/MasterDataVehicleTypesPage.tsx
...
const MasterDataVehicleTypesPage: React.FC = () => {
  const navigate = useNavigate();
  const [vehicleTypes, setVehicleTypes] = useState<MasterData[]>([]);
  ...
  const load = async () => {
    ...
    setErrorMsg("");
    try {
      const data = await listMasterData({ category: "VEHICLE_TYPE" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccessMsg("Vehicle type updated successfully");
      } else {
        await createMasterData(formData);
        setSuccessMsg("Vehicle type created successfully");
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setSuccessMsg("Vehicle type deleted successfully");
      await load();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        ...
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Vehicle Types Management</h2>
          <p style={{ fontSize: 12, color: theme.colors.textMuted }}>Manage vehicle categories and types</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: theme.colors.textMain, marginBottom: 8 }}>
              {editingId ? "Edit Vehicle Type" : "Create Vehicle Type"}
            </div>
...
export default MasterDataVehicleTypesPage;
