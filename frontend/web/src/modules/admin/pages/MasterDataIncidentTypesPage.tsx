// frontend/web/src/modules/admin/pages/MasterDataIncidentTypesPage.tsx
...
const MasterDataIncidentTypesPage: React.FC = () => {
  const navigate = useNavigate();
  const [incidentTypes, setIncidentTypes] = useState<MasterData[]>([]);
  ...
  const load = async () => {
    ...
    setErrorMsg("");
    try {
      const data = await listMasterData({ category: "INCIDENT_TYPE", division: selectedDivision || undefined });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccessMsg("Incident type updated successfully");
      } else {
        await createMasterData(formData);
        setSuccessMsg("Incident type created successfully");
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setSuccessMsg("Incident type deleted successfully");
      await load();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        ...
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Incident Types Management</h2>
          <p style={{ fontSize: 12, color: theme.colors.textMuted }}>Manage incident categories and types</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: theme.colors.textMain, marginBottom: 8 }}>
              {editingId ? "Edit Incident Type" : "Create Incident Type"}
            </div>
...
export default MasterDataIncidentTypesPage;
