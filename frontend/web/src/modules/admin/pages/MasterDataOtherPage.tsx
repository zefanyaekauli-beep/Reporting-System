// frontend/web/src/modules/admin/pages/MasterDataOtherPage.tsx
...
const MasterDataOtherPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const excludedCategories = ["ZONE_TYPE", "INCIDENT_TYPE", "STATUS_TYPE", "VISITOR_CATEGORY", "VEHICLE_TYPE"];
      const filtered = (result || []).filter(
        (item) => !excludedCategories.includes(item.category) && (selectedCategory ? item.category === selectedCategory : true)
      );
      setData(filtered);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccessMsg("Master data updated successfully");
      } else {
        await createMasterData(formData);
        setSuccessMsg("Master data created successfully");
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setSuccessMsg("Master data deleted successfully");
      await load();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        ...
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Other Master Data Management</h2>
          <p style={{ fontSize: 12, color: theme.colors.textMuted }}>Manage other master data categories</p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: theme.colors.background, borderBottom: `1px solid ${theme.colors.border}` }}>
                <th style={{ padding: "12px", textAlign: "left", fontSize: 12, fontWeight: 600, color: theme.colors.textMuted }}>Category</th>
            ...
            <tbody>
              {data.map((item, idx) => (
                ...
                >
                  <td style={{ padding: "12px", fontSize: 13, color: theme.colors.textMain }}>{item.category}</td>
                  ...
                  <td style={{ padding: "12px", fontSize: 13, color: theme.colors.textMuted }}>{item.division || "All"}</td>
...
export default MasterDataOtherPage;
