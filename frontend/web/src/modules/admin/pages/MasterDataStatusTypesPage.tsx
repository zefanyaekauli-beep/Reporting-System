// frontend/web/src/modules/admin/pages/MasterDataStatusTypesPage.tsx
...
const MasterDataStatusTypesPage: React.FC = () => {
  const navigate = useNavigate();
  const [statusTypes, setStatusTypes] = useState<MasterData[]>([]);
  ...
  const load = async () => {
    ...
    setErrorMsg("");
    try {
      const data = await listMasterData({ category: "STATUS_TYPE", division: selectedDivision || undefined });
      setStatusTypes(data || []);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccessMsg("Status type updated successfully");
      } else {
        await createMasterData(formData);
        setSuccessMsg("Status type created successfully");
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this status type?")) return;
    try {
      await deleteMasterData(id);
      setSuccessMsg("Status type deleted successfully");
      await load();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        ...
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Status Types Management</h2>
          <p style={{ fontSize: 12, color: theme.colors.textMuted }}>Manage status categories for reports and tasks</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: theme.colors.textMain, marginBottom: 8 }}>
              {editingId ? "Edit Status Type" : "Create Status Type"}
            </div>
...
export default MasterDataStatusTypesPage;
