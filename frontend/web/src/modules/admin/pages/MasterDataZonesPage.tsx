// frontend/web/src/modules/admin/pages/MasterDataZonesPage.tsx
...
const MasterDataZonesPage: React.FC = () => {
  const navigate = useNavigate();
  const [zones, setZones] = useState<MasterData[]>([]);
  ...
  const divisions = ["SECURITY", "CLEANING", "DRIVER", "PARKING"];

  const load = async () => {
    setLoading(true);
    setErrorMsg("");
    try {
      const [zonesData, sitesData] = await Promise.all([
        listMasterData({ category: "ZONE_TYPE", division: selectedDivision || undefined }),
        listSites(),
      ]);
      setZones(zonesData || []);
      setSites(sitesData || []);
  ...
  const handleEdit = (zone: MasterData) => {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        ...
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Zones Management</h2>
          <p style={{ fontSize: 12, color: theme.colors.textMuted }}>Manage zones and areas for all divisions</p>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ backgroundColor: theme.colors.background, borderBottom: `1px solid ${theme.colors.border}` }}>
                ...
                <th style={{ padding: "12px", textAlign: "left", fontSize: 12, fontWeight: 600, color: theme.colors.textMuted }}>Division</th>
            ...
            <tbody>
              {zones.map((zone, idx) => (
                ...
                >
                  <td style={{ padding: "12px", fontSize: 13, color: theme.colors.textMain, fontWeight: 500 }}>{zone.code}</td>
                  <td style={{ padding: "12px", fontSize: 13, color: theme.colors.textMain }}>{zone.name}</td>
                  <td style={{ padding: "12px", fontSize: 13, color: theme.colors.textMuted }}>{zone.division || "All"}</td>
                  <td style={{ padding: "12px", textAlign: "center", fontSize: 13, color: theme.colors.textMuted }}>{zone.sort_order}</td>
                  <td style={{ padding: "12px", textAlign: "center" }}>
                    <span
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 500,
                        backgroundColor: zone.is_active ? "#D1FAE5" : "#FEE2E2",
                        color: zone.is_active ? "#065F46" : "#DC2626",
                      }}
                    >
...
export default MasterDataZonesPage;
