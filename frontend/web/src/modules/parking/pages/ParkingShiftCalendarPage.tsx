// frontend/web/src/modules/parking/pages/ParkingShiftCalendarPage.tsx
...
interface Shift {
  ...
  siteId: number;
  siteName: string;
  roleType: "SECURITY" | "CLEANING" | "DRIVER" | "PARKING";
  status: ShiftStatus;
  isMine: boolean;
}
...
function ShiftPill({
  ...
  if (shift.status === "OPEN") bgColor = theme.colors.warning;
  if (shift.status === "EXCHANGE_REQUESTED") bgColor = theme.colors.danger;
...
export function ParkingShiftCalendarPage() {
  ...
  async function loadShifts() {
    try {
      setLoading(true);
      setError("");
      const response = await api.get(`/parking/shifts/calendar?start=${rangeStart}&end=${rangeEnd}`);
  ...
  async function callShiftAction(shiftId: number, action: string) {
    try {
      ...
      await api.post(`/parking/shifts/${shiftId}/${action}`);
  ...
  return (
    <MobileLayout title="Kalender Shift Parking">
      <div style={{ padding: "16px", paddingBottom: "80px" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "12px", color: theme.colors.textMuted, margin: "4px 0 0 0" }}>
                    ...
                    return (
                      <td
                        ...
                        style={{
                          ...
                          padding: "4px",
                          verticalAlign: "top",
                          backgroundColor: !isCurrentMonth ? theme.colors.background : "transparent",
                        }}
                      >
                        <div
                          style={{
