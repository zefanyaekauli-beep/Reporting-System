// frontend/web/src/modules/parking/pages/ParkingChecklistPage.tsx
...
export function ParkingChecklistPage() {
  ...
  const loadChecklist = async () => {
    setLoading(true);
    try {
      const res = await api.get("/parking/checklist/today");
  ...
  const handleCompleteItem = async (
    ...
    try {
      const response = await api.patch(`/parking/checklist/${checklist.id}/items/${item.id}/complete`, {
        status,
  ...
  if (loading && !checklist) {
    return (
      <MobileLayout title="Checklist Parking Hari Ini">
        <div
          style={{
            padding: 20,
            textAlign: "center",
    ...
    return (
      <MobileLayout title="Checklist Parking Hari Ini">
        ...
        >
          ...
          <button
            onClick={() => navigate("/parking/attendance")}
            style={{
              marginTop: 16,
              padding: "10px 20px",
