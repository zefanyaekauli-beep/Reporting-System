// frontend/web/src/modules/parking/pages/ParkingReportDetailPage.tsx
...
import { getParkingReport, exportParkingReportPDF, ParkingReport } from "../../../api/parkingApi";

export function ParkingReportDetailPage() {
  const { t } = useTranslation();
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { showToast } = useToast();
  const [report, setReport] = useState<ParkingReport | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setLoading(true);
        const { data } = await getParkingReport(parseInt(id));
        setReport(data);
      } catch (err: any) {
  ...
  const handleExportPDF = async () => {
    ...
    setExporting(true);
    try {
      const blob = await exportParkingReportPDF(parseInt(id));
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Parking_Report_${id}_${new Date().toISOString().split("T")[0]}.pdf`;
