// frontend/web/src/modules/parking/pages/ParkingReportsListPage.tsx
...
import { listParkingReports, exportParkingReportsSummaryPDF, ParkingReport } from "../../../api/parkingApi";
import { useToast } from "../../shared/components/Toast";

type Period = "today" | "week" | "month";

export function ParkingReportsListPage() {
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      const { data } = await listParkingReports({
        site_id: siteId,
        from_date: fromDate,
        to_date: toDate,
      });
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const blob = await exportParkingReportsSummaryPDF({
        site_id: siteId,
        from_date: fromDate,
        to_date: toDate,
      });
      ...
      a.href = url;
      a.download = `Parking_Reports_${fromDate}_${toDate}.pdf`;
  ...
  return (
    <MobileLayout title="Laporan Parkir">
      <SiteSelector
        sites={sites}
        value={siteId}
        ...
        >
          {reports.map((r) => (
            <button
              key={r.id}
              type="button"
              onClick={() => navigate(`/parking/reports/${r.id}`)}
              style={{
                width: "100%",
                textAlign: "left",
