// frontend/web/src/modules/supervisor/pages/Master/Department/DepartmentFormPage.tsx
...
export function DepartmentFormPage() {
  ...
  const loadDepartment = async () => {
    if (!id) return;
    try {
      const response = await api.get(`/master/department/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/master/department/${id}`, formData);
      } else {
        await api.post("/master/department", formData);
      }
      navigate("/supervisor/master/department");
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => navigate("/supervisor/master/department")}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <DashboardCard title="Department Information">
