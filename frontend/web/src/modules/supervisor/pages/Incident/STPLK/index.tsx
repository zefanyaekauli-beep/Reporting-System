// frontend/web/src/modules/supervisor/pages/Incident/STPLK/index.tsx
...
export function STPLKListPage() {
  const navigate = useNavigate();
  const [reports, setReports] = useState<STPLKReport[]>([]);
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      const response = await api.get("/incidents/stplk", { params });
      setReports(response.data);
    } catch (err: any) {
      console.error("Failed to load STPLK reports:", err);
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#002B4B]">NO STPLK (Surat Tanda Laporan Kehilangan)</h1>
        <button
          onClick={() => navigate("/supervisor/incident/stplk/new")}
      ...
      {/* Reports Table */}
      <DashboardCard title="STPLK Reports">
        {loading ? (
              ...
              <tbody>
                {reports.map((report) => (
                  <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">
                      <button
                        onClick={() => navigate(`/supervisor/incident/stplk/${report.id}`)}
