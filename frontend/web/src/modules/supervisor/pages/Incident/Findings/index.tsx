// frontend/web/src/modules/supervisor/pages/Incident/Findings/index.tsx
...
export function FindingsListPage() {
  ...
  const [severityLevel, setSeverityLevel] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    loadReports();
  }, [siteId, incidentDate, status, severityLevel]);
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      if (severityLevel) params.severity_level = severityLevel;

      const response = await api.get("/incidents/findings", { params });
      setReports(response.data);
    } catch (err: any) {
      console.error("Failed to load Findings reports:", err);
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#002B4B]">Findings Report</h1>
        <button
          onClick={() => navigate("/supervisor/incident/findings/new")}
      ...
      <DashboardCard title="Filters">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              value={severityLevel || ""}
              onChange={(e) => setSeverityLevel(e.target.value || undefined)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
            >
      ...
      {/* Reports Table */}
      <DashboardCard title="Findings Reports">
        {loading ? (
                  ...
                  <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">
                      {report.severity_level && (
                        <span className={`px-2 py-1 rounded text-xs font-medium ${getSeverityColor(report.severity_level)}`}>
                          {report.severity_level}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-2 px-3">
                      <button
                        onClick={() => navigate(`/supervisor/incident/findings/${report.id}`)}
