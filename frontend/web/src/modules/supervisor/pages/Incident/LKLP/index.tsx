// frontend/web/src/modules/supervisor/pages/Incident/LKLP/index.tsx
...
export function LKLPListPage() {
  const navigate = useNavigate();
  const [reports, setReports] = useState<LKLPReport[]>([]);
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      const response = await api.get("/incidents/lk-lp", { params });
      setReports(response.data);
    } catch (err: any) {
      console.error("Failed to load LK/LP reports:", err);
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#002B4B]">LK dan LP (Laporan Kejadian)</h1>
        <button
          onClick={() => navigate("/supervisor/incident/lk-lp/new")}
      ...
      {/* Reports Table */}
      <DashboardCard title="LK/LP Reports">
        {loading ? (
              ...
              <tbody>
                {reports.map((report) => (
                  <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">
                      <button
                        onClick={() => navigate(`/supervisor/incident/lk-lp/${report.id}`)}
