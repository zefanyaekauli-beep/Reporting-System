...
if ! lsof -ti:5173 >/dev/null 2>&1; then
    echo -e "${YELLOW}⚠️  Frontend is not running on port 5173${NC}"
    echo "Start the system first with: ./start.sh"
    exit 1
fi

# Kill existing ngrok processes
echo -e "${YELLOW}🛑 Stopping existing ngrok tunnels...${NC}"
...
if [ -n "$FRONTEND_URL" ]; then
    echo -e "   🌐 Public URL: ${GREEN}$FRONTEND_URL${NC}"
    echo ""
...
echo ""
echo "🛑 To stop: ./stop_ngrok.sh or pkill -f 'ngrok http'"
echo ""
